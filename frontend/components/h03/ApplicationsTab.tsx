...
export function ApplicationsTab({ className = "" }: ApplicationsTabProps) {
  ...
  return (
    <div className={className}>
      <ListTemplate
        ...
        dodatkowyPanel={
          <>
            ...
            {actionError && !accepting && !rejecting && <Alert variant="error">{actionError}</Alert>}
      ...
      {/* Świadomy wyjątek: to podgląd danych (✕, klik w tło, Escape, jeden warunkowy przycisk pobrania, który nie zamyka okna), nie para potwierdź/anuluj — ConfirmDialog jej nie modeluje bez zmiany układu przycisków. */}
      ...
      {/* Świadoma zmiana zachowania: dotąd klik w tło zamykał to okno, ConfirmDialog celowo tego nie robi (patrz jego własny komentarz przy tle) — ujednolica z oknem limitu miejsc niżej, które nigdy tak nie działało, i chroni wybraną rolę przed przypadkowym odrzuceniem stray-clickiem. Escape i "Anuluj" nadal zamykają — zmierzone. */}
      <ConfirmDialog open={accepting !== null} title="Akceptuj zgłoszenie" description={accepting ? `${accepting.first_name} ${accepting.last_name}` : undefined} confirmLabel="Akceptuj i wyślij zaproszenie" loading={accepting !== null && pendingId === accepting.id} onConfirm={() => { if (accepting) void acceptApplication(accepting); }} onCancel={() => { setAccepting(null); setActionError(null); }}>{actionError && <Alert variant="error" className="mt-3">{actionError}</Alert>}<Select className="mt-4" label="Rola konta" value={acceptRole} onChange={(event) => setAcceptRole(event.target.value as ApplicationRole)}><option value="volunteer">Wolontariusz</option><option value="student">Student</option><option value="instructor">Prowadzący</option><option value="project_manager">Opiekun Projektu</option></Select></ConfirmDialog>
      {/* Świadoma zmiana zachowania: dotąd klik w tło zamykał to okno, ConfirmDialog celowo tego nie robi — tu dodatkowo chroni przed utratą wpisanego powodu przy przypadkowym kliku obok. Escape i "Anuluj" nadal zamykają — zmierzone. */}
      <ConfirmDialog open={rejecting !== null} title="Odrzuć zgłoszenie" description="Powód jest wymagany." confirmLabel="Odrzuć" loading={rejecting !== null && pendingId === rejecting.id} onConfirm={() => void rejectApplication()} onCancel={() => { setRejecting(null); setActionError(null); }}>{actionError && <Alert variant="error" className="mt-3">{actionError}</Alert>}<label className="mt-4 flex flex-col gap-1.5 text-small font-medium text-ink" htmlFor="reject-reason">Powód<textarea id="reject-reason" rows={4} value={rejectReason} onChange={(event) => setRejectReason(event.target.value)} aria-invalid={rejectError ? true : undefined} className={`rounded-sm border bg-card px-4 py-2.5 text-body text-ink focus-visible:focus-ring ${rejectError ? "border-danger" : "border-line"}`} />{rejectError && <span className="text-caption text-danger" role="alert">{rejectError}</span>}</label></ConfirmDialog>
      <ConfirmDialog open={capacityPrompt !== null && capacityApplication !== null} title="Brak wolnych miejsc" description={capacityPrompt ? `Limit: ${capacityPrompt.capacity}. Aktywnych: ${capacityPrompt.active}. Wnioskowanych: ${capacityPrompt.requested}.` : undefined} confirmLabel="Potwierdź force" loading={capacityApplication !== null && pendingId === capacityApplication.id} onConfirm={() => { if (capacityApplication) void acceptApplication(capacityApplication, true); }} onCancel={() => { setCapacityPrompt(null); setCapacityApplication(null); }}><p className="mt-2 text-small text-muted">Czy świadomie zaakceptować ponad limit?</p></ConfirmDialog>
    </div>
  );
}
