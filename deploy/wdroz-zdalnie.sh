...
#
#   wdroz-zdalnie.sh <pelny-40-znakowy-sha> [--warunki-wstepne]
#
...
#   3. niczego na hoscie nie podnosi ani nie naprawia: mierzy wlasciciela,
#      prawa i wolne miejsce i melduje. O naprawie praw decyduje czlowiek.
#
...
#
# Kody wyjscia:
#   0    - zrobione (albo: same warunki wstepne spelnione)
#   2    - uzycie: brak SHA, SHA nie ma 40 znakow, nadmiarowy argument, brak HOST_BRAMKOWY
...
MASKA="HOST-UKRYTY"
UZYCIE='uzycie: wdroz-zdalnie.sh <pelny-40-znakowy-sha> [--warunki-wstepne]   (adres hosta wylacznie ze zmiennej HOST_BRAMKOWY)'
...
# wlasciciela. Katalog kopii zaklada wlasciciel hosta i to jego decyzja, kto
# ma do niego prawo zapisu.
opisz_katalog() {
...
ZDALNE
    ...
    if [ -z "$(pomiar KONIEC "$wyjscie")" ]; then
        echo "[WARUNKI] NIEZALICZONY: pomiar urwal sie w polowie - brak linii konca pomiaru"
        exit 14
    fi

    local istnieje zapis wlasciciel prawa wolne
    local r_wlasciciel r_prawa r_zapis r_wolne
    istnieje="$(pomiar KOPIE-ISTNIEJE "$wyjscie")"
    zapis="$(pomiar KOPIE-ZAPIS "$wyjscie")"
    wlasciciel="$(pomiar KOPIE-WLASCICIEL "$wyjscie")"
    prawa="$(pomiar KOPIE-PRAWA "$wyjscie")"
    wolne="$(liczba_lub_minus "$(pomiar KOPIE-WOLNE-KB "$wyjscie")")"
    r_wlasciciel="$(pomiar RODZIC-WLASCICIEL "$wyjscie")"
    r_prawa="$(pomiar RODZIC-PRAWA "$wyjscie")"
    r_zapis="$(pomiar RODZIC-ZAPIS "$wyjscie")"
    r_wolne="$(liczba_lub_minus "$(pomiar RODZIC-WOLNE-KB "$wyjscie")")"

    # Po jednej linii na katalog kopii i na jego katalog nadrzedny - to jest
    # to, czym mierzy sie brak prawa zapisu, na ktorym stanelo wdrozenie.
    echo "[WARUNKI] katalog kopii $KATALOG_KOPII: istnieje=$istnieje, wlasciciel=$wlasciciel, prawa=$prawa, zapis=$zapis, wolne=$wolne KB"
    echo "[WARUNKI] katalog nadrzedny $(dirname "$KATALOG_KOPII"): wlasciciel=$r_wlasciciel, prawa=$r_prawa, zapis=$r_zapis, wolne=$r_wolne KB"
    echo "[WARUNKI] uzytkownik wdrazajacy na hoscie: $(pomiar KTO "$wyjscie")"
    ...
    if [ "$a_ok" -eq 0 ]; then
        ...
        echo "[WARUNEK a] praw NIE naprawiam: to decyzja wlasciciela hosta, przyrzad tylko mierzy"
        exit 11
    fi
    ...
    local wolne_do_progu="$wolne"
    [ "$istnieje" = "tak" ] || wolne_do_progu="$r_wolne"
    ...
    local head_hosta
    head_hosta="$(pomiar HEAD "$wyjscie")"
