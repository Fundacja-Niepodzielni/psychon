...
/**
 * Stage E1 (the bridge): the named `keycloak` guard behind
 * `auth:sanctum,keycloak` on business routes. Resolves a LOCAL user by
 * `keycloak_sub`, never touches `realm_access.roles` for authorisation —
 * `users.role` stays the only source (the disagreement guarantee).
 */
class KeycloakGuardTest extends TestCase
{
    ...
    public function test_a_sanctum_token_still_gets_200(): void
    {
