...
| **V8.1.4** Aplikacja wykrywa nienaturalną liczbę żądań i alarmuje o niej. | niespełnione | limity tylko na trzech trasach: `backend/routes/api/sso.php:34`, `backend/routes/api/h03.php:27`, `backend/routes/api/h01.php:34`; brak `RateLimiter::for` i `throttleApi()` (`backend/bootstrap/app.php`) | średnie | Włączyć domyślny limiter dla całego API i osobne limity dla tras publicznych, wgrywania i eksportów. |
